...
type expr = Add of term * expr | Sub of term * expr | T of term
and term = Mult of term * factor | Div of term * factor | F of factor
and factor = Neg of sign | Pos of sign
and sign = Parentheses of expr | Variable of string | Literal of float
...
let unsignedLit =
  ...
  dot >>= function
  | false -> return (Literal (float_of_string whole))
  | true ->
      take_while is_digit >>= fun part ->
      return (Literal (float_of_string (whole ^ "." ^ part)))

let unsignedVar = take_while1 is_alphanum >>| fun v -> Variable v
...
let factor expr =
  ...
  unsignedLit <|> unsignedVar
  <|> ( bracketed '(' (expr <?> "expected expression inside parentheses") ')'
      >>| fun e -> Parentheses e )
  >>| fun v -> if s then Neg v else Pos v
...
and factor = function Neg s -> -.sign s | Pos s -> sign s
...
let parse_expr str =
  ...
  | Done (_pos, e) -> eval e
  | Fail (pos, str) ->
      Printf.printf "Failure on caracter %d\n" pos;
      show_error str;
      0.
  | _ ->
      print_string "something went wrong :s\n";
      0.
